...
6. **レビュー依頼と修正**
   3~5を繰り返す。
7. **PRのマージ**
   メンバー全員の承認が得られたら、PRを`main`ブランチにマージする。マージは以下の手順で行う。
   - Merge Pull Requestを押すと出てくるコミットメッセージをコピー。
   - Squash and mergeに切り替え。
   - commit messageに貼り付ける。
   - Merge!!

   面倒だけど、こうするとコミット履歴が綺麗になる。なお、delete branchについてはやってもやらなくてもOK。一回マージしたブランチは基本的には不要なので消しても大丈夫だが、残しておいても特に影響はない。
