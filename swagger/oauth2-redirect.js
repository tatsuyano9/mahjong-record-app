...
function run() {
  var e,
    ...
    n = a.redirectUrl;
  if (
    ((t = (r = /code|token|error/.test(window.location.hash)
      ? window.location.hash.substring(1).replace("?", "&")
      : location.search.substring(1)).split("&")).forEach(function (e, r, t) {
      t[r] = '"' + e.replace("=", '":"') + '"';
    }),
    (e =
      (r = r
        ? JSON.parse("{" + t.join() + "}", function (e, r) {
    ...
    (r.error &&
      ...
      a.errCb({
        ...
        level: "error",
        message:
          e || "[Authorization failed]: no accessCode received from the server",
      }));
  }
  window.close();
}
